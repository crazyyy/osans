...
if ( !class_exists('RSPluginUpdateChecker_1_3') ):
 ...
 * @access public
 */
class RSPluginUpdateChecker_1_3 {
	public $metadataUrl = ''; //The URL of the plugin's metadata file.
	public $pluginFile = '';  //Plugin filename relative to the plugins directory.
	public $slug = '';        //Plugin slug.
	public $checkPeriod = 48; //How often to check for updates (in hours).
	 ...
	 */
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 48, $optionName = ''){
	...
	public function requestInfo($queryArgs = array()){
		...
		$url = apply_filters('puc_request_url_args-'.$this->slug, $url);
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			$pluginInfo = RSPluginInfo_1_3::fromJson($result['body'], $this->debugMode);
	 ...
	 * @return null
	 */
	public function customRequestInfo($options) {
		$result = wp_remote_get(
			'http://dimsemenov.com/private/update-checker-json.php',
			$options
		);
	}
	...
	public function requestUpdate(){
		...
		return RSPluginUpdate_1_3::fromPluginInfo($pluginInfo);
	}
	...
	public function getUpdateState() {
		...
		if ( !empty($state) && isset($state->update) && is_object($state->update) ){
			$state->update = RSPluginUpdate_1_3::fromObject($state->update);
		}
		return $state;
	}
...
if ( !class_exists('RSPluginInfo_1_3') ):
...
if ( !class_exists('RSPluginUpdate_1_3') ):
...
class RSPluginUpdate_1_3 {
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		$pluginInfo = RSPluginInfo_1_3::fromJson($json, $triggerErrors);
		if ( $pluginInfo != null ) {
